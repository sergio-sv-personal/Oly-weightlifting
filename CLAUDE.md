...
- `src/benchmarks.json` — named CrossFit benchmark WODs (Karen, Fran, …): `{ key: { label, description, scoreType: "time" | "reps" | "rounds" | "load", history: [{ date, score, rx, note? }] } }`. Time scores are `"m:ss"` strings. Renders as the "Benchmarks" board on the Coach summary, directly under Max lifts, in the same last → current / delta grammar (for `time`, a negative delta is the improvement). **Log these as they come up in class** — Sergio doesn't remember historic scores, so there's no backfill; when a class contains a named WOD, append to that benchmark's history (create the key if new) in the same PR as the `crossfitSessions.json` entry. `rx: false` when any part was scaled.
- `src/nutrition.json` — the food log, rendered on the **Fuel** tab. Shape: `{ targets: { kcal, kcalLow, kcalHigh, proteinG, carbsG, fatG, fibreLowG, fibreHighG, preTrainingCarbsG, byDay: { training, rest, double }, source }, days: { "YYYY-MM-DD": { training, fluidsMl, closed, note?, items: [{ name, kcal, p, c, f, fibre, slot? }] } } }`. `slot` is one of `pre | intra | post | veg | fruit | supp` and drives the protocol-adherence row (pre/intra/post are Seb's training-window carbs; veg/fruit his daily habits; supp = pre-workout / night stack, not shown as a tag). `closed: false` marks a day still being logged — it draws as a dashed outline in the chart and shows "so far". Sergio logs food item by item during the day; append to that day's `items`, update `fluidsMl`, and set `closed: true` after the night stack.

  **Standing assumptions — log these without being told:**
  - **The breakfast smoothie has peanut butter in it** — the 40g PB version (879 kcal / 50P / 95C / 32F / 18 fibre), not the 635-kcal no-PB one. Log the no-PB version only when Sergio says he skipped it, or when Claude has deliberately planned a no-PB day for the fat budget — and in that case **say so in the reply and confirm before closing the day**, because a plan saying "no PB today" is not evidence he followed it. Getting this wrong is a 244 kcal / 20g fat error that misdirects the rest of the day's advice.
  - **Intra Gatorade, 2 scoops (130 kcal / 30C / 750ml)** on every training day. Three scoops only for a double session, and only if he says so.
  - The **night stack** (15 kcal / 1C / 3 fibre) closes the day, but wait for him to confirm he's taken it.
  - If a training day is about to close without a pre/intra/post item, **flag it** rather than assuming it was skipped — the usual misses are the second protein water, the veg, and the smoothie.

  Label values where a package exists; mark estimates in the item name. The older markdown ledgers in `docs/nutrition/` are the archive of the first tracked weeks.
